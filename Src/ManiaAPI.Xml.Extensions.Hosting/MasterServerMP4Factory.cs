...
public interface IMasterServerMP4Factory
{
    ...
    /// <summary>
    /// Creates a <see cref="MasterServerMP4"/> client for the specified master server. This method does not require <see cref="RequestWaitingParamsAsync(string?, CancellationToken)"/> to be called.
    /// </summary>
    ...
    /// <summary>
    /// Creates a <see cref="MasterServerMP4"/> client for the specified master server name. Requires <see cref="RequestWaitingParamsAsync(string?, CancellationToken)"/> to be called first (at least once at the start of the application).
    /// </summary>
    ...
    /// <summary>
    /// Creates a <see cref="MasterServerMP4"/> client for the first master server in the list. Requires <see cref="RequestWaitingParamsAsync(string?, CancellationToken)"/> to be called first (at least once at the start of the application).
    /// </summary>
    /// <returns>The created client.</returns>
    MasterServerMP4 CreateClient();

    /// <summary>
    /// Creates <see cref="MasterServerMP4"/> clients for all available master servers. Requires <see cref="RequestWaitingParamsAsync(string?, CancellationToken)"/> to be called first (at least once at the start of the application).
    /// </summary>
